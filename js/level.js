...
const Level = (() => {
  ...
  const spawns = {
    player: { x: 5, floor: 32 },
    signs: [
      { x: 8,  floor: 32, text: '←→／A D 移動，Space 跳躍，F 法杖揮擊。你身上只有「火」之靈印——其餘元素的靈印散落於洞窟深處，拾取即可習得。' },
      { x: 17, floor: 32, text: '按 1火 4土 輸入已習得的元素，Enter 起陣。同元素不能相鄰；被克元素不能接續（水克火・土克水・風克土・火克風）。序列首尾必須接成圓。' },
      { x: 26, floor: 32, text: '速成陣式——「1 4」火土＝火種（燃燒地帶）；「4 1」土火＝爆破陷阱。習得新元素後，按 Tab 查看更多陣式。' },
      { x: 44, floor: 32, text: '速陣快而弱，強陣強而慢。施展窗口越長，越容易被人破陣。' },
      { x: 58, floor: 32, text: '習得「水」後可組治療陣——「2 1 4」水火土＝泉湧（火被水/土夾住而透明化，只作橋接）。' },
      { x: 76, floor: 32, text: '結界擋路！破陣＝把對方每個元素換成克制它的元素。此陣為 土→火；風克土、水克火，故破陣序列為「風 水」。站到結界前，按住 Shift 輸入 3、2，再按 Shift+Enter。' },
      { x: 98, floor: 32, text: '裂谷太寬，跳不過去。風系陣法可以位移——「3 2」風水＝流體位移，或「3 4 1」風土火＝衝刺（土是間接元素，只作橋接）。空中也能起陣。' },
      { x: 154, floor: 32, text: '前方有咒師。他們佈陣時序列清晰可見——在施展窗口內以 Shift 輸入克制序列破其陣！注意：反噬陣被破會反傷（陣心呈暗紅色）。' },
    ...
    boss: { x: 203, floor: 32 },
    pickups: [
      { x: 14,  floor: 32, type: 'elem', elem: 'earth' }, // 起始洞窟：土之靈印（佈陣教學前）
      { x: 55,  floor: 32, type: 'elem', elem: 'water' }, // 走廊：水之靈印
      { x: 74,  floor: 32, type: 'elem', elem: 'wind' },  // 結界室一：風之靈印（破陣需風水）

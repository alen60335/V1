...
// --- 擴充內容：新頭目與新結界的序列 ---
eq(F.validate([風, 土, 火]), null, 'Boss2 衝鋒陣 合法');
eq(F.validate([土, 水, 風]), null, 'Boss2 落雷陣 合法');
eq(F.validate([火, 土, 水, 風, 土, 水]), null, 'Boss3 大爆裂（六元素）合法');
a = F.analyze([火, 土, 水, 風, 土, 水]);
eq([a.effective[0], a.effective[1]], [火, 水], 'Boss3 大爆裂 主效果 火/水');
eq(a.backlash, false, 'Boss3 大爆裂 無反噬（可走位迴避）');
eq(F.validate([水, 火, 風, 水, 風, 土]), null, 'Boss3 反噬自癒（六元素）合法');
a = F.analyze([水, 火, 風, 水, 風, 土]);
eq(a.backlash, true, 'Boss3 反噬自癒 構成反噬');
eq(a.effective[0], 水, 'Boss3 反噬自癒 主效果為水（治療）');
eq(F.breakSeqOf([水, 火, 風, 水, 風, 土]), [土, 水, 火, 土, 火, 風], 'Boss3 自癒破陣 → 土水火土火風');
eq(F.validate([土, 水, 火, 風, 土, 火]), null, '結界四（六元素）合法');
eq(F.breakSeqOf([土, 水, 火, 風, 土, 火]), [風, 土, 水, 火, 風, 水], '結界四破陣 → 風土水火風水');
eq(F.analyze([火, 風, 水, 風, 土]).backlash, true, '結界三為反噬結界');
// 破陣序列自身仍為合法陣
eq(F.validate(F.breakSeqOf([火, 土, 水, 風, 土, 水])), null, 'Boss3 大爆裂破陣序列自身合法');
eq(F.validate(F.breakSeqOf([土, 水, 火, 風, 土, 火])), null, '結界四破陣序列自身合法');
